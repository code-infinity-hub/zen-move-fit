                ...
                <div class="d-flex flex-column px-4 py-5 h-100 overflow-y-auto">
                  ...
                  >
                    ...
                    <div class="d-flex ga-1 text-caption text-grey-lighten-1 mb-4">
                      ...
                      <template v-if="currentExercise.load.type !== 'bodyweight'">
                        <span>•</span>

                        <span>
                          {{
                            currentExercise.load.type === "fixed" ?
                              `${currentExercise.load.value} ${currentExercise.load.unit}` :
                              t("$vuetify.message.percent_of_your_weight", currentExercise.load.value)
                          }}
                        </span>
                      </template>
                    </div>
                    ...
                    >{{ t("$vuetify.label.start_training") }}</v-btn>
                    <template v-else-if="!resting">
                      <div
                        v-if="currentExercise?.load?.type !== 'bodyweight'"
                        class="mb-3"
                      >
                        <TextField
                          is-money
                          hide-symbol
                          density="compact"
                          :model-value="executedLoad ?? ''"
                          :label="t('$vuetify.label.executed_load')"
                          :suffix="currentExercise?.load?.unit ?? 'kg'"
                          @update:model-value="executedLoad = $event !== '' && $event != null ? Number($event) : null"
                        />
                      </div>
                  ...
                  <template v-if="workout.exercises">
                      ...
                      >
                        ...
                        <div class="flex-fill d-flex flex-column align-start ga-1">
                          ...
                          <p class="text-caption text-grey-lighten-1 d-flex ga-1">
                            ...
                            <template v-if="exercise.load.type !== 'bodyweight'">
                              <span>•</span>

                              <span>
                                {{
                                  exercise.load.type === "fixed" ?
                                    `${exercise.load.value} ${exercise.load.unit}` :
                                    t("$vuetify.message.percent_of_your_weight", exercise.load.value)
                                }}
                              </span>
                            </template>
                          </p>
...
const executedLoad = ref<number | null>(null);
...
const currentExercise = computed(() => workout.value?.exercises?.[indexExercise.value - 1]);
...
const videoSources = computed(() => [{ src: videoUrl.value, type: "" }]);
...
watch(currentExercise, (ex) => {
  const load = ex?.load;
  executedLoad.value = load && load.type !== "bodyweight" && load.value != null ? load.value : null;
});

watch(() => executionCurrentExercise.value.series.length, () => {
  const load = currentExercise.value?.load;
  executedLoad.value = load && load.type !== "bodyweight" && load.value != null ? load.value : null;
});
  ...
  return workoutService.calculateTotalCalories(
    workoutService.buildActivityTimeline(
      ...
      workout.value!.exercises!.map((exercise) => ({
        ...exercise,
        rest_seconds: isLastExerciseFromSet.value ? exercise.rest_after_exercise_seconds : exercise.rest_seconds
      })),
      account.value
    )
  );
...
const createDefaultExercise = (ind: number = 0) => {
  const exercise = workout.value!.exercises[ind]!;
...
const startTraining = async (fromZero = true) => {
  ...
  if (fromZero) {
    ...
    activity.value.workout_id = workout.value!._id;
  ...
  await Promise.all([
    persistentSession.set("type-training", "TRAINING"),
  ...
  if (permission === "granted") {
    await geolocationService.startBackgroundGeolocation(
      t("$vuetify.message.training_in_progress"),
      t("$vuetify.message.your_workout_is_active")
    );
...
const bodyNotification = () => {
  ...
  return {
    ...
    text: `${currentExercise.value?.name} • ${formatTime(activity.value.duration_seconds)} • ${formatNumber(kcal.value, "decimal")} ${t("$vuetify.label.kcal_abbreviation")}${extraMessage}`,
...
const finishSerie = () => {
  ...
  serie!.load = executedLoad.value;

  isLastExerciseFromSet.value = (currentExercise.value!.sets - 1) === (series.length - 1);

  timeResting.value = isLastExerciseFromSet.value ? currentExercise.value!.rest_after_exercise_seconds : currentExercise.value!.rest_seconds;
  resting.value = true;
};
...
const nextStep = (jump: boolean) => {
  ...
  const time = isLastExerciseFromSet.value ? currentExercise.value!.rest_after_exercise_seconds : currentExercise.value!.rest_seconds;
  ...
  if (series[currentExercise.value!.sets - 1]?.rest?.ended_at) {
    const nextExercise = workout.value!.exercises[indexExercise.value];
